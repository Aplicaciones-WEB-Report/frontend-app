...
export default {
  ...
  methods: {
    async cargarMisPostulaciones() {
      ...
      if (!currentUser) {
        alert("Por favor, inicia sesión para ver tus postulaciones.");
        return;
      }
      try {
        const [myAppsResponse, jobOffersResponse, messagesResponse] = await getMyApplicationsData(currentUser.id);
