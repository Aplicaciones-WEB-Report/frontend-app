...
export default {
  ...
  methods: {
    cargarMisPostulaciones() {
      ...
      this.misPostulaciones = storedApplications.map(app => ({
    ...
    eliminarPostulacion(applicationId) {
      if (!confirm("¿Seguro que quieres eliminarla de tu vista?")) return;
      let stored = JSON.parse(localStorage.getItem('my-applications')) || [];
      stored = stored.filter(app => app.applicationId !== applicationId);
      localStorage.setItem('my-applications', JSON.stringify(stored));
...
<template>
  <div class="mis-postulaciones-container">
    ...
    <div class="controls">
      <input v-model="filtroTitulo" type="text" :placeholder="$t('buscar_titulo_placeholder')" class="search-input"/>
    </div>
    <div class="postulaciones-table">
      ...
      <template v-if="postulacionesFiltradas.length > 0">
        <template v-for="postulacion in postulacionesFiltradas" :key="postulacion.applicationId">
          ...
          <div class="table-cell" :data-label="$t('columna_resultado')"><span class="status-badge">{{ postulacion.resultado }}</span></div>
      ...
      <div v-else class="no-data">{{ $t("sin_postulaciones") }}</div>
    </div>
