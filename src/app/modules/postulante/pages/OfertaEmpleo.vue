...
export default {
  ...
  methods: {
    async cargarOfertas() {
      this.isLoading = true;
      try {
        const [offersResponse, myAppsResponse] = await getJobBoardData(this.currentUserId);
    ...
    async postular(jobOfferId) {
      const payload = {
        job_offer_id: String(jobOfferId),
        candidate_id: String(this.currentUserId),
      ...
      try {
        ...
        this.myAppliedIds.add(String(jobOfferId));
