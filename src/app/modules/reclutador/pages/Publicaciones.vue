  ...
  methods: {
    async cargarPublicaciones() {
      try {
        ...
        // Filtramos en el frontend para mostrar solo las del usuario logueado
        const misPublicaciones = response.data.filter(pub => pub.employer_id === currentUser.id);

        this.publicaciones = misPublicaciones;
    ...
    async guardarPublicacion() {
      try {
        ...
        if (this.formulario.id) {
          await updatePublication(this.formulario.id, this.formulario);
          alert('¡Publicación actualizada exitosamente!');
        } else {
          const payload = { ...this.formulario, employer_id: currentUser.id };
          delete payload.id;
  ...
  <div class="reclutador-publicaciones">
      ...
      <template v-if="publicacionesPaginadas.length > 0">
        <template v-for="publicacion in publicacionesPaginadas" :key="publicacion.id">
          ...
          <div class="data-cell" :data-label="$t('columna_estado')">
            <span :class="['status-badge', publicacion.status === 'Activa' || publicacion.status === 0 ? 'status-active' : 'status-draft']">
              {{ (publicacion.status === 0 || publicacion.status === 'Activa') ? 'Activa' : 'Borrador' }}
            </span>
          </div>
          <div class="data-cell" :data-label="$t('columna_aplicaciones')">{{ publicacion.applicationCount || 0 }}</div>
    ...
    <div v-if="modalVer" class="modal">
      <div class="modal-content">
        ...
        <p><strong>{{ $t("estado") }}:</strong> {{ (publicacionSeleccionada.status === 0 || publicacionSeleccionada.status === 'Activa') ? 'Activa' : 'Borrador' }}</p>
